...
int RREFont::charWidthNoSort(uint8_t c, int *_xmin) 
{
  ...
  uint16_t recIdx = ((rFont->offs[c-rFont->firstCh]));
  uint16_t recNum = ((rFont->offs[c-rFont->firstCh+1]))-recIdx;
...
int RREFont::charWidthOptim(uint8_t c, int *_xmin)
{
  ...
  switch(rFont->type & 0xf) {
    ...
    case RRE_P16B:
      chWd = (rFont->rects+lastIdx*2)+1;
      break;
#endif
#if ENABLE_RRE_P8B==1
    case RRE_P8B:
      chWd = ((rFont->rects+lastIdx)&0xf)+1;
      break;
#endif
  }
  return chWd;
...
bool RREFont::isdigit(uint8_t ch)
{
	if((ch >= 0x30) && (ch <= 0x39))
		return true;
	else
